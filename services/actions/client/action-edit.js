class ActionEdit extends HTMLElement {
  ...
  attributeChangedCallback (name, oldValue, newValue) {
    if (name === 'action-id' && newValue !== oldValue) {
  ...
  connectedCallback () {
    ...
    const form = this.shadowRoot.querySelector('action-form');
    if (form) {
      form.addEventListener('action-saved', (e) => {
  ...
  show () {
    if (!this.isOpen()) {
      ...
      // Update form with action-id if set
      if (this.actionId) {
  ...
  open (actionId) {
    if (actionId) {
      ...
      const form = this.shadowRoot.querySelector('action-form');
      if (form) {
        form.removeAttribute('action-id');
      }
      // Set new action-id
      this.actionId = actionId;
      this.setAttribute('action-id', actionId);
    }
    this.show();
  }
  ...
  updateForm () {
    const form = this.shadowRoot.querySelector('action-form');
    console.log('ActionEdit: updateForm called', { form: !!form, actionId: this.actionId });
    if (form && this.actionId) {
      // Set the attribute, which will trigger the form's attributeChangedCallback
      console.log('ActionEdit: Setting action-id on form', this.actionId);
      form.setAttribute('action-id', this.actionId);
    } else if (this.actionId && this.isOpen()) {
  ...
  render () {
    ...
    // Update form if action-id is already set
    if (this.actionId) {
      this.updateForm();
    }
  }
...
customElements.define('action-edit', ActionEdit);
