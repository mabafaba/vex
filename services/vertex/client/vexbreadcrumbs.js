class VexBreadcrumbs extends HTMLElement {
    ...
    async fetchAncestryData() {
        ...
        try {
            const response = await fetch('/vex/vertex/' + this.state.vexId + '/ancestry');
            if (!response.ok) throw new Error(`Failed to fetch ancestry: ${response.statusText}`);
    ...
    render() {
        ...
        this.shadowRoot.innerHTML = `
            ...
            <div class="breadcrumbs-container">
                ${ancestryData.map((item, index) => {
                    return `
                        <span class="breadcrumb-link" data-vex-id="${item.id}">${item.content.substring(0, 20)}</span>
                        ${index < ancestryData.length - 1 ? '<span class="breadcrumb-separator">></span>' : ''}
                    `;
                }).join('')}
            </div>
        `;
        ...
        const breadcrumbLinks = this.shadowRoot.querySelectorAll('.breadcrumb-link');
        breadcrumbLinks.forEach(link => {
            const vexId = link.getAttribute('data-vex-id');
            link.addEventListener('click', (event) => this.handleCrumbClick(event, vexId));
        });
    }
